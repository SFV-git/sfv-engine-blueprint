...
## ACCOUNT / BRAND
- SFV_UGC final name (name pending Will decision)
- SFV_404 platform (own IG? are.na? both?)
- Alias burner account details (off Engine, Will only)

## HARDWARE
- Three monitor setup (planned or already confirmed?)
- Exact motherboard model and NVMe slot count
- RAM upgrade timeline

## TOOLS
- Scheduling tool for 8 IG accounts (Later vs Buffer vs other)
- Canva vs alternative for Level 6+ graphics generation
- Remote access solution (Tailscale recommended, not confirmed)
- Docker (yes or no)
- n8n implementation phase (when exactly)
- Whisper (local on R&D terminal vs API)

## WORKFLOWS
- abbass catch logic (auto-detected or manually flagged?)
- SFV_EVENTS delivery method (Pixieset like Studio or different?)
